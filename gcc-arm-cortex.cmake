GET_FILENAME_COMPONENT(CORTEX_CMAKE_DIR ${CMAKE_CURRENT_LIST_FILE} DIRECTORY)
SET(CMAKE_MODULE_PATH ${CORTEX_CMAKE_DIR} ${CMAKE_MODULE_PATH})

SET(CORTEX_SUPPORTED M0 M0P M3 M4 CACHE INTERNAL "cortex supported families")
IF(CORTEX_TYPE)
    SET(CORTEX_TYPE "${CORTEX_TYPE}" CACHE STRING "Cortex chip to build for")
...
FUNCTION(CORTEX_ADD_HEX_BIN_TARGETS TARGET)
...
FUNCTION(CORTEX_ADD_DUMP_TARGET TARGET)
    IF(EXECUTABLE_OUTPUT_PATH)
        SET(FILENAME "${EXECUTABLE_OUTPUT_PATH}/${TARGET}")
    ELSE()
        SET(FILENAME "${TARGET}")
...
FUNCTION(CORTEX_PRINT_SIZE_OF_TARGETS TARGET)
    IF(EXECUTABLE_OUTPUT_PATH)
        SET(FILENAME "${EXECUTABLE_OUTPUT_PATH}/${TARGET}")
    ELSE()
        SET(FILENAME "${TARGET}")
...
FUNCTION(CORTEX_SET_LINKER_SCRIPT TARGET LINKER_SCRIPT)
    CONFIGURE_FILE(${LINKER_SCRIPT} ${CMAKE_CURRENT_BINARY_DIR}/${TARGET}.ld)
    GET_TARGET_PROPERTY(TARGET_LD_FLAGS ${TARGET} LINK_FLAGS)
    IF(TARGET_LD_FLAGS)
        SET(TARGET_LD_FLAGS "\"-T${CMAKE_CURRENT_BINARY_DIR}/${TARGET}.ld\" ${TARGET_LD_FLAGS}")
    ELSE()
        SET(TARGET_LD_FLAGS "\"-T${CMAKE_CURRENT_BINARY_DIR}/${TARGET}.ld\"")
    ENDIF()
